#!/bin/bash
# lista de arquivos kitti - cvlibs
...
# verifica se foi interrompido (deve existir um arquivo zip completo ou parcialmente baixado)
last_started=`ls | grep _sync | awk -F'_' '{print $1"_"$2"_"$3"_"$4"_"$5}'`
if [ "$last_started" != "" ]; then
        # se sim, calcula a linha no array para continuar
        linha=`echo ${files[@]/$last_started//} | cut -d/ -f1 | wc -w | tr -d ' '`
        # pega o subarray a partir da linha identificada
        n_files=(${files[@]:$linha})
        # echo $last_started "entrou"
else
        # se nao, baixa todos os arquivos
        linha=0
        n_files=(${files[@]:$linha})
        # echo $linha "nao entrou"
fi;
